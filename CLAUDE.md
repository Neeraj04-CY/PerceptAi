...
- `simulation.py` — deterministic in-memory fakes: the ONE substrate for unit tests (via `tests/conftest.py`) and the simulated benches (`evals/reasoning_bench.py`, `evals/workforce_bench.py`). Simulated sessions run the real runtime/world model/reasoning against scripted screens; `fast_config` disables the UIA provider so tests never observe the real desktop. `FakeSpecialist` + `ScriptedMissionPlanner` + `build_simulated_workforce` do the same for the workforce layer.
